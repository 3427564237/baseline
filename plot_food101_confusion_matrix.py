...
def find_latest_confusion_matrix_csv():
    candidates = list(OUTPUT_ROOT.glob("food101_subset50*/food101_subset50_*_confusion_matrix.csv"))
    if not candidates:
        raise FileNotFoundError(
            "No Food-101 subset confusion_matrix.csv file was found in outputs/food101_subset50*."
        )
...
def plot_heatmap(matrix, class_names, output_path, normalized):
    if normalized:
        ...
        title = "Food-101 50-Class Subset Confusion Matrix (Normalized)"
        value_text = "proportion"
    else:
        data = matrix
        color_label = "Sample count"
        title = "Food-101 50-Class Subset Confusion Matrix"
        value_text = "count"
    ...
    for index, class_name in enumerate(class_names):
        ...
        ax.text(index, index, label, ha="center", va="center", color="black", fontsize=6)
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot a heatmap from a saved Food-101 subset confusion_matrix.csv file."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=None,
        help="Path to a confusion_matrix.csv file. Defaults to the latest Food-101 subset output.",
    )
    parser.add_argument(
        "--normalized",
        action="store_true",
