...
def find_latest_per_class_csv():
    candidates = list(OUTPUT_ROOT.glob("food101_subset50*/food101_subset50_*_per_class_accuracy.csv"))
    if not candidates:
        raise FileNotFoundError(
            "No Food-101 subset per_class_accuracy.csv file was found in outputs/food101_subset50*."
        )
...
def plot_per_class_accuracy(rows, output_path):
    ...
    ax.set_title("Food-101 50-Class Subset Per-Class Accuracy")
    ax.set_xlabel("Class")
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot a bar chart from a saved Food-101 subset per_class_accuracy.csv file."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=None,
        help="Path to a per_class_accuracy.csv file. Defaults to the latest Food-101 subset output.",
    )
    parser.add_argument(
        "--output",
        type=Path,
